...
One of my favorite use cases for epoxy's templating functions in Shiny
is to store the data for a template in a data frame.
Then, the apps's inputs are combined in a reactive expressions
that filters down to a single row of the data frame.
Finally, that row is sent via `render_epoxy()` to the UI,
where it's dynamically injected into the template.

...
Read more about inline styles in `?epoxy_transform_inline`.
...
style transformers or with `epoxy_transform()`.

<!-- FIXME -->
````
```{epoxy, epoxy_style = epoxy_transform("bold")}`r ''`
...
<!-- FIXME -->
```{epoxy, epoxy_style = epoxy_transform("bold")}
All cars stopped between {min(cars$dist)} and {max(cars$dist)} feet
...
<!-- FIXME -->
````
```{epoxy, epoxy_style = epoxy_transform("collapse")}`r ''`
...
```{epoxy, epoxy_style = epoxy_transform("collapse")}
- The first three letters are {letters[1:3]*}.
- When capitalized, they are {LETTERS[1:3]&}.
- They're indexed by {1:3|}.
```
...
Because `epoxy_transform_html()` is applied first,

...
- .class, .style:
  ...
  Classes and inline style directives added to the `<epoxy-html>`
  container into which the elements in `...` are placed.

- .item_tag, .item_class:

  The HTML element tag name and classes used to wrap each template
  variable. By default, each template is wrapped in a `<span>`.
...
- .class_item:
  ...
  **\[deprecated\]** Deprecated in epoxy v1.0.0, please use
  `.item_class` instead.

- .container:

  **\[deprecated\]** Deprecated in epoxy v1.0.0, where the container is
  now always `<epoxy-html>`.

- .container_item:

  **\[deprecated\]** Deprecated in epoxy v1.0.0, please use `.item_tag`
  instead.

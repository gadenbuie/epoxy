...
Report prepared on 2023-05-21 from `inline-reporting.Rmd` with knitr
version 1.42 😆. Read more about
...
combination of `dplyr::group_by()` and `dplyr::summarize()` here.)
...
`ref.label` and providing a different slice of data via the `data` chunk
option.

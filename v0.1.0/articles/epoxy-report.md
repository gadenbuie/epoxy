...
````
    ...
    ```{=html}
      <li>
        <strong class="car-make">Chevrolet</strong>
        <em class="car-model">Malibu</em>
      </li>
      <li>
        <strong class="car-make">Dodge</strong>
        <em class="car-model">Caravan</em>
      </li>
      <li>
        <strong class="car-make">Ford</strong>
        <em class="car-model">Expedition</em>
      </li>
    ```
    </ul>
...
Because
[`epoxy_transform_html()`](http://pkg.garrickadenbuie.com/epoxy/v0.1.0/reference/epoxy_transform_html.md)
is applied first, `{{.uppercase manufacturer}}` is assumed to be a CSS
class and not an inline transformer class.
...
You can get access the inline formatter by adding one additional layer
of `{{ }}`.
...
This works even if you don’t use the HTML formatter. In the last
example, `{{ {{.uppercase manufacturer }} }}` would produce `CHEVROLET`.

### Raw LaTeX chunks

Similarly, you can also use `epoxy_latex` chunks to epoxy R and LaTeX
together. By default, expressions in these types of chunks are wrapped
in `<` and `>`.
